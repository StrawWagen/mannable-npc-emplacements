...
local clipDecayVar = CreateConVar( "npcmannedemplacement_clipdecay", 120, FCVAR_ARCHIVE, "Change how fast the NPC MANNABLE EMPLACEMENT clips decay, default 120s" )
...
local function NpcLikes( Npc1, Npc2 )
    ...
    if not Npc1.Disposition then return false end
...
function ENT:PlyEnterEmplacement( Ply )
    ...
    net.Start( "npcmanned_TurretBlockAttackToggle" )
    net.WriteBit( true )
    net.Send( Ply )
...
function ENT:EmplAssignNpc( NPC )
    ...
    if not IsNpc or IsValid( self.EmplUser ) then return end
...
function ENT:Think()
    ...
    if not myTbl.IsDead then
        ...
        if UserIsNpc or myTbl.UserWasNpc then
            ...
            local DeadNpc = not validUser or myTbl.EmplUser:Health() <= 0
            if not DeadNpc then
            ...
            if myTbl.NextBigThink < cur then -- big think, find, manage npc users
                myTbl.NextBigThink = cur + 0.5

                if ( DeadNpc or DistToEmpl > 1000 or IncapableOfManning ) and myTbl.FreeGun == false then
                    if IncapableOfManning and validUser then
                        self:EmplBlacklist( myTbl.EmplUser )
                    end
                    self:EmplClear()
                    myTbl.UserWasNpc = false
                    validUser = IsValid( myTbl.EmplUser ) -- user was cleared!

                elseif not DeadNpc then
                    myTbl.UserWasNpc = true

                    local Enemy = myTbl.EmplUser:GetEnemy()
                    local isCalm = true -- npcs dont leave emplacements if they're "calm"
                    local ValidEnemy = IsValid( Enemy )
                    local CanShootEnemy = false
                    local EmplManningDist = myTbl.ManningDist

                    if ValidEnemy then
                        CanShootEnemy = self:EmplacementCanShoot( Enemy:GetPos() )
                        isCalm = ( myTbl.EmplUser.Health and myTbl.EmplUser.GetMaxHealth ) and ( myTbl.EmplUser:Health() >= myTbl.EmplUser:GetMaxHealth() ) -- not calm if we lost health
                        isCalm = isCalm and Enemy:GetPos():Distance( myTbl.EmplUser:GetPos() ) > 150 -- not calm if they're right next to us
                        isBored = math.abs( myTbl.lastFire - cur ) > 3
                        if math.abs( myTbl.lastFire - cur ) > 10 and self:Visible( Enemy ) then -- not calm if we haven't shot them in >8s
                            isCalm = false
                    ...
                    if CorrectSequence and DistToEmpl < EmplManningDist then -- kick this npc OUT!
                        myTbl.EmplUser.IncorrectSequenceCount = math.Clamp( myTbl.EmplUser.IncorrectSequenceCount + 1, 0, 40 )

                    elseif not CorrectSequence and DistToEmpl < EmplManningDist then
                        myTbl.EmplUser.IncorrectSequenceCount = math.Clamp( myTbl.EmplUser.IncorrectSequenceCount + -1, 0, 40 )
                    ...
                    if DistToEmpl <= EmplManningDist and CanEnterEmplacement then
                        myTbl.EmplUser:SetNPCState( NPC_STATE_SCRIPT ) -- here because of dynsquads
                        -- re-mann
                    ...
                    local InDanger = sound.GetLoudestSoundHint( 8, myTbl.EmplUser:GetPos() ) -- DANGER!

                    --flee the emplacement
                    if InDanger and myTbl.EmplUser:GetNPCState() == NPC_STATE_SCRIPT and DistToEmpl <= EmplManningDist then
                    ...
                    if not CanShootOrCalm and myTbl.EmplUser:GetActiveWeapon() ~= NULL and DistToEmpl <= EmplManningDist and validUser and IsValid( Enemy ) and myTbl.EmplUser:GetNPCState() == NPC_STATE_SCRIPT then
                        self:ExitEmplacement( myTbl.EmplUser )
                        --print( "badang" )
                    ...
                    if not InDanger and DistToEmpl > EmplManningDist and validUser and CanEnterEmplacement and not myTbl.EmplUser:IsCurrentSchedule( SCHED_FORCED_GO_RUN ) and myTbl.EmplUser:GetNPCState() ~= NPC_STATE_SCRIPT then
                    ...
                    if DistToEmpl > EmplManningDist and validUser and myTbl.EmplUser:GetNPCState() == NPC_STATE_SCRIPT then
                        self:ExitEmplacement( myTbl.EmplUser )
                        --print( "far" )

                    elseif DistToEmpl > EmplManningDist or not validUser then
                        self:ExitEmplacement( myTbl.EmplUser )
                        --print( "far2" )

                    end

                    if validUser then-- hacky pyramid, makes squads of npcs give emplacements to the squad leader
            ...
            if validUser then -- aiming, shooting!
                local Enemy = myTbl.EmplUser:GetEnemy()
                local EnemyHealth = 0
                local ValidEnemy = IsValid( Enemy )
                local CanShootEnemy = false
                ...
                if ValidEnemy then
                    ...
                    local hitEnt = canSeeResult.Entity
                    local hitEnemy = ( hitEnt == Enemy )
                    userCanSeeEnemy = not canSeeResult.Hit or hitEnemy
                    if not userCanSeeEnemy then -- innacuracy when the user cant directly see the enemy
                        local x = ( cur % 2 ) - 1
                        local y = ( cur % 4 ) - 2
                        local z = ( cur % 3 ) - 1.5
                    ...
                    local notSeeEnemyBoredTolerance = 2
                    ...
                    local sinceSeen = ( cur - myTbl.emplacementLastSeen )

                    WantsToFire = sinceSeen < notSeeEnemyBoredTolerance and not ( sinceSeen > 10 and ( cur % 4 ) <= 3 )

                end
                if not DeadNpc and myTbl.EmplaceActive and myTbl.EmplaceManned and myTbl.NextSmallThink < cur then -- small think!
                    myTbl.NextSmallThink = cur + 0.08

                    if CanShootEnemy and myTbl.EmplaceActive and EnabledAi() then -- make allies want to take cover
                        ...
                        if IsValid( tr.Entity ) then
                            ...
                            if tr.Entity:IsNPC() and not myTbl.ClearFiringLine and tr.Entity.NextForcedCover < cur then
                                tr.Entity:SetSaveValue( "vLastKnownLocation", tr.HitPos )
                                tr.Entity:SetSchedule( SCHED_TAKE_COVER_FROM_ORIGIN )
                                tr.Entity.NextForcedCover = cur + 1
                            end
                        end
                ...
                if not DeadNpc and myTbl.AimingAtEnemy and EnemyHealth > 0 and CanShootEnemy and WantsToFire and myTbl.ClearFiringLine then
        ...
        elseif UserIsPly then -- ply aiming!
            local StandPos = self:GetEmplacementStandPos()
            local DistToEmpl = 0
        ...
        if myTbl.EmplInputFire and myTbl.NextFire < cur then
    ...
    elseif not myTbl.Inactive then -- mechanics thinking, eg dying
...
-- takes 80 explosive damage to kill
function ENT:OnTakeDamage( damage )
    if not damage:IsExplosionDamage() then return end
    if damage:GetDamage() <= 80 then return end
...
function ENT:EmplFireThink( Fire )
    ...
    if DoReload and self.Reloading == nil then
        ...
        ent:Spawn()
        ent:SetCollisionGroup( 11 )
        SafeRemoveEntityDelayed( ent, clipDecayVar:GetInt() )
        ent.DoNotDuplicate = true
...
function ENT:EmplBlacklist( ent )
    if not IsValid( ent ) then return end
...
local function WaitToGiveLeaderAChance( ent )
    ...
    ent.JustTakeTheDamnEmplacementTime = CurTime() + 6
...
local function IsValidManner( ent )
    ...
    local SeqId = ent:LookupSequence( "Man_Gun" )
...
function ENT:SearchForPersonToManIt()
    ...
    if IsValid( self.NpcToManTheTurret ) and not IsValid( self.EmplUser ) then
        ...
        if not IsAlive and IsClose then
            self:EmplBlacklist( self.NpcToManTheTurret )
            self.NpcToManTheTurret:SetSchedule( SCHED_RUN_RANDOM ) -- get out the way!
    ...
    elseif self.FreeGun and not IsAlreadyManning( self.NpcToManTheTurret ) then
        ...
        for _, ent in ipairs( NearbyNpcs ) do
            positions[ent] = ent:GetPos()

        end
        table.sort( NearbyNpcs, function( a, b ) -- sort areas by distance to nextbot
            local ADist = positions[a]:DistToSqr( StandPos )
            local BDist = positions[b]:DistToSqr( StandPos )
            return ADist < BDist
